...
final class StyleValidator
{
    ...
    private function validValue(StyleDefinition $definition, mixed $value): bool
    {
        return match ($definition->type) {
            ...
            'length' => is_int($value) || is_float($value) || $this->isStructuredLength($value) || (is_string($value) && preg_match('/^(auto|-?\d+(\.\d+)?(px|rem|em|%|vw|vh))(\s+-?\d+(\.\d+)?(px|rem|em|%|vw|vh)){0,3}$/', $value) === 1),
    ...
    private function isStructuredLength(mixed $value): bool
    {
        return is_array($value)
            ...
            && in_array($value['unit'] ?? null, ['px', 'rem', 'em', '%', 'vw', 'vh'], true);
    }
}
