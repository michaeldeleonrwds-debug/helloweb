...
final class MediaAssetController extends Controller
{
    ...
    {
        return response()->json(['media' => $request->user()->mediaAssets()->where('status', 'active')->latest()->get()->map(fn (MediaAsset $asset): array => $this->data($asset))->values()]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json(['media' => $this->data($asset)], 201);
    }
    ...
    public function show(MediaAsset $asset): JsonResponse
    {
        ...
        return response()->json(['media' => $this->data($asset)]);
    }
    ...
    private function data(MediaAsset $asset): array
    {
        return ['id' => $asset->id, 'originalFilename' => $asset->original_filename, 'mimeType' => $asset->mime_type, 'fileSize' => $asset->file_size, 'width' => $asset->width, 'height' => $asset->height, 'altText' => $asset->alt_text, 'status' => $asset->status];
    }
}
